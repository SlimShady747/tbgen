...
class TestbenchGenerator(object):
	...
	def __init__(self, vfile_name=None, ofile_name=None):
		...
		if vfile_name == None:
			sys.stderr.write(
				"ERROR: You did not provide an input file name.\n")
			sys.exit(1)
		else:
			self.open()
		self.parser()
		...
		try:
			if(ofile_name == None):
				if(self.ofile_name == None):
					...
					print(
						"You did not specify an output file name, using '%s'." % ofname)
				else:
	...
	def print_dut(self):
		...
		align_cont = self.align_print(
			list(map(lambda x: ("", "." + x[1], "(", x[1], '),'), self.pin_list)), 4)
	...
	def print_wires(self):
		self.printo(self.align_print(
			list(map(lambda x: (x[3], x[2], x[1], ';'), self.pin_list)), 4))
		self.printo("\n")
	...
	def print_module_head(self, timescale, iname):
		if iname is None:
			iname = '%s.v' % self.mod_name
...
''')
	...
	aparse = argparse.ArgumentParser(
		description='Automatically generate Verilog testbench')
	aparse.add_argument('input_file',  help='input Verilog file')
	aparse.add_argument(
		'output_file', help='output Verilog testbench', nargs='?', default=None)
	aparse.add_argument('-p', '--period', type=int,
						help='set period in clock ticks (default=10)', default=10)
	aparse.add_argument(
		'-t', '--timescale', help='set timescale (default=1ns/10ps)', default='1ns/10ps')
	aparse.add_argument(
		'-d', '--dumpfile', help='set dumpfile (default=tb_output.vcd)', default='tb_output.vcd')
	aparse.add_argument('-l', '--level', type=int,
						help='set dump depth level (usually 0,1, or 2; default=2)', default=2)
	aparse.add_argument('-r', '--resetneg', help='set reset to negative (default positive)',
						action='store_const', const=1, default=0)
	aparse.add_argument(
		'-i', '--include', help='sets user include file name (default=<name>.v)', default=None)
	args = aparse.parse_args()

	tbg = TestbenchGenerator(args.input_file, args.output_file)

	tbg.print_module_head(args.timescale,args.include)
	tbg.print_wires()
	tbg.print_dut()
